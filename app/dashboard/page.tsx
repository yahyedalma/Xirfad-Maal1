...
import { Award, BookOpen, CheckCircle2, GraduationCap, LayoutDashboard, LogOut, Menu, PlayCircle, Settings, ShieldCheck, UserRound, Sparkles, ArrowRight } from 'lucide-react';
...
type Certificate = { id: string; course_id: string; verification_id: string | null; course_title?: string | null; issued_at?: string | null };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [profileRes, enrollmentRes, certificateRes] = await Promise.all([
        ...
        supabase.from('certificates').select('id,course_id,verification_id,course_title,issued_at').eq('student_profile_id', auth.user.id).eq('revoked', false).order('issued_at', { ascending: false }),
      ]);
  ...
  const readyCourses = courses.filter((course) => course.progress === 100 && !certificates.some((certificate) => certificate.course_id === course.course_id));
    ...
    <div className="student-shell">
      ...
      <div className="student-main">
        ...
        <div className="student-content">
          ...
          {readyCourses.length > 0 && <section className="certificate-ready-banner"><div className="certificate-ready-icon"><Sparkles size={20}/></div><div><strong>🎉 Hambalyo! Shahaadadaadu waa diyaar.</strong><p>Waxaad dhammaystirtay {readyCourses.length === 1 ? 'koorso' : 'koorsooyin'}. Shahaadadaada ka eeg bogga Shahaadooyinka.</p></div><a href="/certificates">Eeg shahaadada <ArrowRight size={16}/></a></section>}
          ...
          <section className="student-grid-stats">
            ...
            <div className="student-stat"><div className="student-stat-top"><div className="student-stat-icon"><Award size={18}/></div><small>AWARDS</small></div><strong>{certificates.length}</strong><span>Shahaadooyin la helay</span></div>
          </section>
          ...
          <section className="student-section" id="my-courses">
            ...
            {courses.length ? <div className="student-course-list">{courses.map((course) => <article className="student-course" key={course.id}>
              ...
              <div><h3>{course.courses?.title ?? 'Koorso'}</h3><div className="student-course-meta">{course.completedLessons} / {course.totalLessons} cashar • {course.progress === 100 ? 'La dhammeeyay' : course.status}</div><div className="student-progress-row"><div className="student-progress"><span style={{width:`${course.progress}%`}}/></div><span className="student-progress-label">{course.progress}%</span></div></div>
              <div className="student-course-actions"><a className="student-mini-btn" href={`/courses/${course.course_id}`}>{course.progress === 100 ? 'Dib u eeg' : course.progress ? 'Sii wad' : 'Bilow'}</a>{course.progress === 100 && certificates.some((c) => c.course_id === course.course_id) && <a className="student-cert-mini" href="/certificates" title="Eeg shahaadada"><Award size={15}/></a>}</div>
          ...
          <div className="student-lower">
            ...
            <section className="student-section" id="certificates"><div className="student-section-head"><div><h2>Shahaadooyinka</h2><p>Guulaha aad kasbatay.</p></div></div><div className={`certificate-card ${certificates.length ? 'has-certificate' : ''}`}><div className="certificate-card-glow"/><Award size={22}/><h3>{certificates.length ? 'Hambalyo! 🎉' : 'Shahaadadaada sugaysa'}</h3><p>{certificates.length ? `Waxaad haysataa ${certificates.length} shahaado oo diyaar ah.` : 'Dhammaystir 100% koorsada si shahaadada si automatic ah loo abuuro.'}</p><a className="btn btn-primary" href={certificates.length ? '/certificates' : '#my-courses'} style={{background:'#fff',color:'#0b3a91'}}>{certificates.length ? 'Eeg shahaadooyinka' : 'Sii wad waxbarashada'}</a></div>{certificates.length > 0 && <div className="certificate-list">{certificates.slice(0,3).map((certificate) => <a className="certificate-item" href="/certificates" key={certificate.id}><div><b>{certificate.course_title || 'Certificate'}</b><span style={{display:'block'}}>{certificate.verification_id || 'Certificate ID'}</span></div><Award size={16} color="#1764e9"/></a>)}</div>}</section>
          </div>
        </div>
      </div>
    </div>
